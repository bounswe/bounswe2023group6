...
const PostCard = ({ post, currentUser, onUpvote, onDownvote }) => {
  ...
  useEffect(() => {
    const checkCurrentUserCreator = () => {
      if (post && currentUser) {
        const updatedIsCurrentUserCreator = post.creatorUser.username === currentUser.username;
        setIsCurrentUserCreator(updatedIsCurrentUserCreator);
      }
    };
