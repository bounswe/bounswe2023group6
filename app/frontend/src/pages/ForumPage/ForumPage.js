...
export default function ForumPage() {
	...
	const handleUpvote = async (postId) => {
		if (!localStorage.getItem('username')) {
			setError('You must be logged in to upvote a post.')
		} else {
			try {
				const response = await upvotePost(postId)
				setFilteredPosts((prevPosts) => prevPosts.map((post) => (post.postId === postId ? response.data : post)))
			} catch (error) {
		...
		<>
			...
			<div className='w-full flex flex-col'>
				<div className='w-full flex justify-center p-4 bg-neutral-100 pb-12'>
					<div className='hidden md:flex flex flex-col w-1/6'></div>
						...
						<div className='h-18 w-full flex flex-row pt-2 center-items'>
							...
							<div className='w-full h-1/2 flex justify-end px-2'>
								<div className='dropdown dropdown-end'>
									...
									<ul
										tabIndex='0'
										className='dropdown-content bg-neutral-800 text-neutral-200 z-[1] menu p-2 shadow rounded-box w-52'
									>
										<li>
											<a onClick={handleSortChange}>Oldest</a>
										</li>
										<li>
											<a onClick={handleSortChange}>Newest</a>
										</li>
									</ul>
								</div>
							</div>
					...
					<div className='hidden md:flex w-1/6'>
						<div className='flex flex-col h-full w-full py-8 px-1 rounded'>
								...
								<button
									className={`mx-2 px-4 py-2 rounded ${
										selectedCategory === 'all'
											...
											: 'bg-neutral-300 text-neutral-600 hover:text-white hover:bg-cyan-900'
									}`}
							...
							<SelectTags tags={tags} selectedTags={selectedTags} handleTagChange={handleTagChange} />
						</div>
					</div>
				</div>
