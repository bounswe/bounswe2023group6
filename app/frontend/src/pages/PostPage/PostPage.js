...
const PostPage = () => {
	...
	const handleUpvote = async () => {
		if (!localStorage.getItem('username')) {
			setError('You must be logged in to upvote or downwote a post.')
		} else {
			try {
	...
	const handleDownvote = async () => {
		if (!localStorage.getItem('username')) {
			setError('You must be logged in to upvote or downvote a post.')
		} else {
			try {
	...
	const handleUpvoteComment = async (commentId) => {
		if (!localStorage.getItem('username')) {
			setError1('You must be logged in to upvote or downvote a comment.')
		} else {
			try {
	...
	const handleNewComment = async (e) => {
		if (!localStorage.getItem('username')) {
			setError2('You must be logged in to comment.')
		} else {
			e.preventDefault()
			try {
		...
		<>
			...
			<div className='w-full flex flex-col bg-neutral-100 items-center pt-8'>
				<div className='w-3/4 flex flex-col p-2 pb-20'>
					{post && <PostCard post={post} onUpvote={handleUpvote} onDownvote={handleDownvote} currentUser={currentUser} />}
