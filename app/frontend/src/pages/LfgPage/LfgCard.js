...
const LfgCard = ({ group, currentUser }) => {
	...
	const leaveGroup = async () => {
		if (!localStorage.getItem('username')) {
			setError2('You must be logged in to leave a group.')
		} else {
			console.log(group)
			console.log(isMember)
			axiosInstance.defaults.withCredentials = true
			await axiosInstance
				.post(`/lfg/${group.lfgId}/leave`, {
					withCredentials: true
				})
				.then((response) => {
					console.log(123)
	...
	return (
		<div key={group.lfgId} className='card compact bg-neutral-200 text-neutral-800 shadow-xl m-2 p-2'>
			<div className='absolute top-2 right-2 flex'>
				{isCurrentUserCreator && <EditLfg lfg={group} />}
			...
			<div className='flex-col'>
				...
				<div className='flex justify-between items-center'>
					...
					{error2 && <p className='text-red-500'>{error}</p>}
				</div>
			</div>
		</div>
	)
